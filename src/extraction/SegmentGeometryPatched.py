          ...
          if segmentID == segmentNode:
            ...
            if np.count_nonzero(slicetemp) == 0:
              ...
              try:
                ...
                # Fallback for degenerate/collinear slices that qhull cannot process
                x_range = (max(coords_Ijk[0]) - min(coords_Ijk[0])) * PixelWidthMm
                y_range = (max(coords_Ijk[1]) - min(coords_Ijk[1])) * PixelWidthMm
                Fdiam = max(x_range, y_range)
